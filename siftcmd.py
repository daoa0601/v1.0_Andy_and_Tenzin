...
# -----------------------------------------------------------------------------------------
    ...
    def exec_cmd(self, cmd_req_struct, request_hash):
        ...
        # dnl
            ...
            if not self.check_fdname(fname):
                ...
                cmd_res_struct['result_1'] = self.res_reject
                cmd_res_struct['result_2'] = 'This is refering to a directory not an existing file'
            else:
                file = open(fname, 'rb')
                file_content = file.read()
                file_length = len(file_content)
                hash_fn = SHA256.new()
                hash_fn.update(file_content)
                file_hash = hash_fn.digest()
                
                cmd_res_struct['result_1'] = self.res_accept
                cmd_res_struct['result_2'] = file_length
    ...
    def exec_upl(self, filename):
        ...
        try:
            if self.check_fdname(filename): 
                upload = SiFT_UPL(self.mtp)
                upload.handle_upload_server(path)
            else:
                raise SiFT_UPL_Error('File name or directory name is empty, starts with . or contains unsupported characters')
                
        except SiFT_MTP_Error as e:
            raise SiFT_UPL_Error('Unable to upload --> ' + e.err_msg)
    ...
    def exec_dnl(self, filename):
        ...
        try:
            if not self.check_fdname(filename):
                raise SiFT_DNL_Error('File name or directory name is empty, starts with . or contains unsupported characters')
            if not exists(path):
                raise SiFT_DNL_Error('This is refering to a directory not an existing file')
            else:
                download = SiFT_DNL(self.mtp)
                download.handle_download_server(path)
               
                
        except SiFT_MTP_Error as e:
            raise SiFT_DNL_Error('Unable to download --> ' + e.err_msg)
