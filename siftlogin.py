...
class SiFT_LOGIN:
    ...
    def handle_login_server(self):
        ...
        timecheck = int(login_req_struct['timestamp'])
        timecurrent = time.time_ns()

        # Checking if the timestamp is within an acceptable time:
        if timecheck < (timecurrent - 10**9) or timecheck > (timecurrent + 10**9):
        ...
        server_rand = Random.get_random_bytes(16).hex()
        # building login response
        login_res_struct = {}
        login_res_struct['request_hash'] = request_hash
        login_res_struct['server_random'] = server_rand
        ...
        client_rand = login_req_struct['client_random']
        
        key_final_init = bytes.fromhex(client_rand + server_rand)

        key_final = HKDF(key_final_init, 32, request_hash, SHA256, 1)

        self.mtp.key_set(key_final)
    ...
    def handle_login_client(self, username, password):
        ...
        key_final_init = bytes.fromhex(login_req_struct['client_random']) + login_res_struct['server_random']
        key_final = HKDF(key_final_init, 32, request_hash, SHA256, 1)
        self.mtp.key_set(key_final)
